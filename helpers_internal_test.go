// Whitebox: shared test fixtures for the package-pdf (whitebox) test files
// and for fuzz_test.go's seed corpus. These build Values and Readers by
// poking unexported fields (stream, dict, objptr, Reader.f/end/trailer)
// directly, which only package pdf itself can do; blackbox tests use their
// own equivalent builders in helpers_test.go, built only from exported API.
...
// page inside an object stream, indexed by a cross-reference stream.
